...
def compute_after_tax_amount(hh, year, common, prices):
    ...
    for who, p in enumerate(hh.sp):
        p.fin_assets['unreg'].amount_after_tax_real = (
            hh_tax.sp[who].disp_inc - hh_tax_0.sp[who].disp_inc)
...
def file_household(hh, year, common, prices):
    ...
    hh_tax = []  # persons in household (1 or 2)
    for p in hh.sp:
    ...
    if hh.couple:
        hh_tax = srd.Hhold(hh_tax[0], hh_tax[1], prov=hh.prov)
    else:
        hh_tax = srd.Hhold(hh_tax[0], prov=hh.prov)
...
def file_household_amount_to_tax(hh, year, common, prices):
    ...
    for p in hh.sp:
        p = srd.Person(
            ...
            age=p.age, earn=real_2018(p.earn), rpp=real_2018(p.rpp),
            cpp=real_2018(p.cpp), othntax=real_2018(p.othntax),
    ...
    if hh.couple:
        hh_tax = srd.Hhold(hh_tax[0], second=hh_tax[1], prov=hh.prov)
    else:
        hh_tax = srd.Hhold(hh_tax[0], prov=hh.prov)
    common.tax.compute(hh_tax)
    return hh_tax

...
class table: 
    def __init__(self,prov='qc',scenario='M',gender='males',web=False):
        ...
        quotes = pd.read_excel(self.params+'/quotients.xlsx',
                               sheet_name='DTH_Tx_Scenario_'+scenario,skiprows=3) 
        provdict = {    
            'nl':10,
            'pe':11,
            'ns':12,
        ...
        self.splice()
        return
    # north west territories given saskatchewan retrospective mortality cause too short history. 
    # newfoundland and labrador given new brunswick for same reason
    def pull_history_web(self,prov='qc',gender='males'):  
        maprov = {'qc': 'que','on': 'ont','bc': 'bco','ab': 'alb','nl': 'nbr',
                  'pe':'pei','ns':'nsc','nb':'nbr','mb':'man','sk':'sas',
                  'nt':'sas','yu':'yuk'}
        file = 'http://www.prdh.umontreal.ca/BDLC/data/'+maprov[prov]+'/Mx_1x1.txt'
        r = requests.get(file)
        ...
        self.history.to_excel(self.params+'/'+prov+'_'+gender+'.xlsx')
        return
    def pull_history(self,prov='qc',gender='males'):  
        self.history = pd.read_excel(self.params+'/'+prov+'_'+gender+'.xlsx')
        self.history.index = [str(i) for i in range(1921,2012)]
        return
    def splice(self):
    ...
    def sx(self,byear,agestart,agestop):
        ...
        for i in range(0, nyrs):
            if (yrstart+i<2063):
                ...
                prob *= (1.0-self.tab.loc['2062',str(agestart+i)])
        return prob
    ...
    def ann_old(self,byear,agestart,rate):
        n = 1.0
        for i in range(agestart+1,111):
    ...
    def ann(self, byear, agestart, rate):
        ...
        for i in range(1, 111 - agestart):
            if yrstart + i < 2063:
                prob_surv *= (1.0-self.tab.loc[str(yrstart+i-1), 
                                               str(agestart+i-1)])
            else:
                prob_surv *= (1.0-self.tab.loc['2062', str(agestart+i-1)])
